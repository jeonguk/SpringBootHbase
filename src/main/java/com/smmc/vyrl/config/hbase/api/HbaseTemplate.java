...
import com.smmc.vyrl.config.constant.MiddleWare;
import com.smmc.vyrl.config.hbase.dto.EventLog;
    ...
    public <T> List<T> find(String tableName, final Scan scan, final RowMapper<T> action) {
        return this.execute(tableName, new TableCallback<List<T>>() {
            ...
            public  List<T> doInTable(Table table) throws Throwable {
                ...
                // 캐싱이 설정되지 않은 경우(기본값은 1)，기본값으로 구성 5000
                if (caching == 1) {
                    scan.setCaching(5000);
                }
        ...
        try {
            ...
            if (null != mutator) {
                try {
                    ...
                    mutator.close();
                    sw.stop();
                    LOGGER.info(EventLog.buildEventLog(EventType.middleware_opt, MiddleWare.HBASE.symbol(), sw.getTotalTimeMillis(), status, "hbase reuqest").toString());
                } catch (IOException e) {
